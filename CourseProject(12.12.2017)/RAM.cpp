...
{
	Assembly_Byte *temp_elector = new Assembly_Byte[size_elector];
	for (int i = 0; i <= size_elector - 1; ++i)
	{
		temp_elector[i] = elector[i];
	}
	delete[]elector;
	elector = new Assembly_Byte[++size_elector];
	for (int i = 0; i <= size_elector - 1; ++i)
	{
...
{
	Assembly_Byte *temp_data = new Assembly_Byte[size];
...
void RAM::push(const Assembly_Byte &value)
{
	if (data == nullptr) 
	{
		data = new Assembly_Byte[++size];
...
void RAM::push(const Double_Word &value)
{
	if (data == nullptr) 
	{
		size += 4;
...
int RAM::make_double_word() 
{
	Double_Word dw(data[size - 4], data[size - 3], data[size - 2], data[size - 1]);
	int temp = dw.convert_into_int();
	return temp;
}
